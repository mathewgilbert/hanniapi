export function scalarHTML(jsonPath, cfg = {}) {
    const title = cfg.title || 'Miaw API';
...
<html>
  <head>
    <title>${title}</title>
    <meta name="description" content="${title} Fast With Elysia.js"/>
    <meta name="og:description" content="${title} Fast With Elysia.js"/>
    <meta charset="utf-8" />
  ...
  <body>
    <script
      id="api-reference"
      data-url="${jsonPath}"
      data-configuration='{"spec":{"url":"${jsonPath}"},"_integration":"elysiajs"}'
    >
    </script>
